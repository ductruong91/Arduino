...
SoftwareSerial mybluetooth(2, 3); //khai báo tên bluetooth và chân tx , rx
...
const int R_S = A4; //cb dò line trái
...
void setup() {
 ...
 mybluetooth.begin(9600);
  ...
  analogWrite(motorAspeed, 60); // tốc độ động cơ a ban đầu 120 ( 0 - 255)
  analogWrite(motorBspeed, 60);// tốc độ động cơ b ban đầu 120 ( 0 - 255)
  delay(2000);
...
void loop() {
  ...
  if(mybluetooth.available()>0){
    doc_gia_tri = mybluetooth.read();
    Serial.println(doc_gia_tri);
    Serial.println("");
